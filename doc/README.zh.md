...
- 标准 Linux 主线可识别的 `virtio-mmio + virtio-net` 设备路径，当前通过 `-net loopback` 暴露，并已有仓库内裸机 loopback 冒烟覆盖。
- 整机 snapshot 保存 / 恢复。
...
- 标准 `virtio,mmio` 传输层和其后的可选 `virtio-net` 设备，可通过 `-net loopback` 启用当前仓库内 loopback 后端
- 整机 snapshot
...
- `-net <off|loopback>`：挂接标准 `virtio-mmio + virtio-net` 设备。`off` 表示该传输层不暴露网卡；`loopback` 启用当前仓库内的 loopback 后端
